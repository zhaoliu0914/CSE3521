...
def calc_global_joint_prob(model, variableValues):
  ...
  probability = 1
  for varName in variableValues:
    varValue = variableValues[varName]
    #print(f"varName = {varName}")
    #print(f"varValue = {varValue}")

    var_probability = read_cpt(model, varName, variableValues)
    #print(f"var_probability = {var_probability}")

    if varValue == False:
      var_probability = 1 - var_probability

    probability = probability * var_probability
...
def calc_query_exact_brute(model, queryVar, queryVal, evidence):
  ...
  for jptEntry in truefalse_combination_iterator(model.vars):
    ...
    query_with_evidence = {queryVar: queryVal}
    query_with_evidence.update(evidence)
    is_subset_QE = dict_issubset(jptEntry, query_with_evidence)
    if is_subset_QE:
      pr_QE = pr_QE + pr_entry

    is_subset_E = dict_issubset(jptEntry, evidence)
    if is_subset_E:
      pr_E = pr_E + pr_entry
...
def calc_query_exact_tree(model, queryVar, queryVal, evidence):
  ...
  l1_norm = prQ_T + prQ_F
  if queryVal:
    probability = prQ_T / l1_norm
  else:
    probability = prQ_F / l1_norm
...
def recurse_calc_query_exact_tree(model, queryVar, evidence, variableValues, remainingCalc):
  ...
  if marginalize:
    ...
    #
    # (Reference solution is 7 lines of code.)
    remainingCalc = remainingCalc[1:]
    variableValues[var] = True
    varT_prR_T, varT_prR_F = recurse_calc_query_exact_tree(model, queryVar, evidence, variableValues, remainingCalc)
    variableValues[var] = False
    varF_prR_T, varF_prR_F = recurse_calc_query_exact_tree(model, queryVar, evidence, variableValues, remainingCalc)

    variableValues[var] = None

    prQ_T = varT_prR_T + varF_prR_T
    prQ_F = varT_prR_F + varF_prR_F
    ...
    if queryVar in model.varDist[var].parents:
      ...
      initial_query_var_value = variableValues[queryVar]
      variableValues[queryVar] = True
      temp_prQ_T = read_cpt(model, var, variableValues)

      variableValues[queryVar] = False
      temp_prQ_F = read_cpt(model, var, variableValues)

      variableValues[queryVar] = initial_query_var_value
      ...
      var_probability = read_cpt(model, queryVar, variableValues)

      prQ_T = var_probability
      prQ_F = 1 - var_probability
      ...
      var_probability = read_cpt(model, var, variableValues)
      var_value = variableValues[var]
      if var_value == False:
        var_probability = 1 - var_probability
      prQ_T = var_probability * prQ_T
      prQ_F = var_probability * prQ_F
